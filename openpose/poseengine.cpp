...
namespace op {
	...
	{
		std::string prototxt_file = "/home/yfji/OpenposeEasy/openpose/pose_deploy_linevec.prototxt";
		std::string caffemodel_file = "/home/yfji/OpenposeEasy/openpose/pose_iter_440000.caffemodel";
	...
	void PoseEngine::connectBodyPartsCpu(vector<float>& poseKeypoints, const float * const heatMapPtr, const float * const peaksPtr, const Size & heatMapSize, const int maxPeaks, const int interMinAboveThreshold, const float interThreshold, const int minSubsetCnt, const float minSubsetScore, const float scaleFactor, vector<int>& keypointShape)
	{
				...
				for (auto i = 1; i <= nA; i++)
				{
					for (auto j = 1; j <= nB; j++)
					{
						...
						if (normVec > 1e-6)
						{
							...
							{
								const auto mX = min(heatMapSize.width - 1, int(sX + lm*dX / numInter+0.5));
								const auto mY = min(heatMapSize.height - 1, int(sY + lm*dY / numInter+0.5));
	...
	void PoseEngine::nms(CaffeBlob<float> * bottom_blob, CaffeBlob<float> * top_blob, float threshold)
	{
	...
	void PoseEngine::renderKeypointsCpu(Mat & frame, const vector<float>& keypoints, vector<int> keyshape, const std::vector<unsigned int>& pairs, const std::vector<float> colors, const float thicknessCircleRatio, const float thicknessLineRatioWRTCircle, const float threshold, float scale_x, float scale_y)
	{
		...
		{
			{
				...
				const auto thicknessRatio = max(int(std::sqrt(area)*thicknessCircleRatio * ratioAreas+0.5f), 1);
				...
				for (auto pair = 0u; pair < pairs.size(); pair += 2)
				{
					...
					if (keypoints[index1 + 2] > threshold && keypoints[index2 + 2] > threshold)
					{
						...
						const cv::Point keypoint1{ intRound(keypoints[index1] * scale_x), intRound(keypoints[index1 + 1] * scale_y) };
						const cv::Point keypoint2{ intRound(keypoints[index2] * scale_x), intRound(keypoints[index2 + 1] * scale_y) };
				...
				for (auto part = 0; part < numberKeypoints; part++)
				{
					...
					if (keypoints[faceIndex + 2] > threshold)
					{
						...
						const cv::Scalar color{ colors[(colorIndex + 2) % numberColors],
							colors[(colorIndex + 1) % numberColors],
							colors[(colorIndex + 0) % numberColors] };
						const cv::Point center{ intRound(keypoints[faceIndex] * scale_x), intRound(keypoints[faceIndex + 1] * scale_y) };
	...
	void PoseEngine::renderPoseKeypointsCpu(Mat & frame, const vector<float>& poseKeypoints, vector<int> keyshape, const float renderThreshold, float scale_x, float scale_y, const bool blendOriginalFrame)
	{
	...
	void PoseEngine::keypointsFromImage(cv::Mat & im, cv::Mat& canvas, vector<float>& keypoints, vector<int>& keypointShape)
	{
		...
		CaffeBlob<float> nms_out(1, 56, POSE_MAX_PEOPLE + 1, 3);
		CaffeBlob<float> input(1, 57, baseSize.height,baseSize.width);
		float* input_data = input.data;
		for (auto i = 0; i < blob.channels; ++i) {
			cv::Mat heatmapOrigin(baseSize.height,baseSize.width, CV_32F, input_data);
			cv::resize(heatmaps[i], heatmapOrigin, baseSize, cv::INTER_CUBIC);
